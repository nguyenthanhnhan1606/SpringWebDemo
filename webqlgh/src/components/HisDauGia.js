...
const HisDauGia = () => {
    ...
    const loadOrder = async () => {
        try {
            let e = `${endpoints['getOrderByidShip']}/${user.id}`;
    ...
    const handleBidSubmit = async (e, orderId, trangthai) => {
        e.preventDefault();
        if (trangthai === "Đang giao") {
            window.confirm("Đơn hàng đang trong trạng thái Đang giao. Không thể đấu giá.");
            return;
        }
            ...
            {orders.map(order => (
                <Card key={order.id} className="mb-4">
                    <Card.Body>
                        ...
                        {(order.trangthai === "Hoàn thành"||order.trangthai === "Hoàn hàng" || order.trangthai==="Đang giao") ? null : (
                        <ListGroup.Item>
                            ...
                            <InputGroup className="mt-2">
                                ...
                                />
                                <Button variant="info" onClick={(e) => handleBidSubmit(e, order.id, order.trangthai)}>
                                    Đấu giá
                                </Button>
                            </InputGroup>
                        </ListGroup.Item>
